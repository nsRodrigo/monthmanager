...
function CardDetail() {
  const { year: y, month: m, cardId } = Route.useParams();
  const year = Number(y);
  ...
  const monthInst = installments
    ...
    .filter((i) => {
      ...
      return pur?.cardId === cardId;
    });

  const total = monthInst.reduce((s, i) => s + i.amount, 0);
  const paidTotal = monthInst.filter((i) => i.paid).reduce((s, i) => s + i.amount, 0);
  const fullyPaid = isCardFullyPaid(installments, purchases, cardPayments, cardId, year, month);

  // Group installments by their parent purchase, so the user sees compras + parcelas.
  const grouped = new Map<string, { purchase: typeof purchases[number]; items: Installment[] }>();
  ...
  if (!card) {
    return (
      <div className="mx-auto max-w-3xl px-5 py-12">
        <Link to="/meses/$year/$month" params={{ year: y, month: m }} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="h-4 w-4" /> Voltar
        </Link>
        <p className="mt-8 text-center text-muted-foreground">Cartão não encontrado.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8 md:py-12">
      ...
      >
        <ArrowLeft className="h-4 w-4" /> {MONTHS[month]} {year}
      </Link>
      ...
      <header className="mb-6 overflow-hidden rounded-2xl border border-border bg-gradient-card p-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Fatura</p>
            <h1 className="text-2xl font-bold tracking-tight">{card.name}</h1>
          </div>
          <button
            onClick={() => setCardPaid.mutate({ cardId, year, month, paid: !fullyPaid })}
            className={`rounded-full px-4 py-2 text-xs font-semibold transition-colors ${
              fullyPaid ? "bg-success/15 text-success hover:bg-success/25" : "bg-warning/15 text-warning hover:bg-warning/25"
            }`}
          >
        ...
        <div className="mt-5 grid grid-cols-3 gap-3">
          ...
          <Stat label="A pagar" value={formatCurrency(total - paidTotal)} tone={total - paidTotal > 0 ? "warning" : "success"} />
        </div>
      </header>
      ...
      <div className="space-y-4">
        {groups.map(({ purchase, items }) => {
          ...
          return (
            <div key={purchase.id} className="overflow-hidden rounded-2xl border border-border bg-card">
              <div className="flex items-start justify-between gap-3 border-b border-border p-4">
                <div className="min-w-0">
                  <p className="truncate font-semibold">{purchase.description}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Compra em {formatDate(purchase.date)} · Total {formatCurrency(purchase.totalAmount)}
                ...
                {items.map((i) => (
                  <div key={i.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <button onClick={() => setEditing(i)} className="flex-1 min-w-0 text-left">
                      <p className={`text-sm font-medium ${i.paid ? "text-muted-foreground line-through" : ""}`}>
                    ...
                    <button onClick={() => setEditing(i)} className="text-muted-foreground hover:text-primary" title="Editar parcela">
      ...
      <EditInstallmentDialog
        ...
        onDeleteParent={editingPurchase ? () => removePurchase.mutate(editingPurchase.id) : undefined}
      />
    </div>
  );
}
